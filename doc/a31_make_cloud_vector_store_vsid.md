...
### 1. 個別Vector Store作成
- 6種類のデータセットから個別にVector Storeを作成（CC NewsとLivedoorの各3種類）
- データセット種別:
  - `a02_cc_news`: CC NewsデータセットQ&A（基本生成方式 - LLM生成）
  - `a02_livedoor`: LivedoorデータセットQ&A（基本生成方式 - LLM生成）
  - `a03_cc_news`: CC NewsデータセットQ&A（カバレッジ改良方式 - ルールベース生成）
  - `a03_livedoor`: LivedoorデータセットQ&A（カバレッジ改良方式 - ルールベース生成）
  - `a10_cc_news`: CC NewsデータセットQ&A（ハイブリッド生成方式）
  - `a10_livedoor`: LivedoorデータセットQ&A（ハイブリッド生成方式）
...
1. **a02_cc_news** (L85-95): CC NewsデータセットQ&A（基本生成方式 - LLM生成）
   - ファイル: `qa_output/a02_qa_pairs_cc_news.csv`
   - テキストカラム: `question`
   - チャンクサイズ: 2000, オーバーラップ: 100
   - 最大30MB, 4000チャンク

2. **a02_livedoor** (L96-106): LivedoorデータセットQ&A（基本生成方式 - LLM生成）
   - ファイル: `qa_output/a02_qa_pairs_livedoor.csv`
   - テキストカラム: `question`
   - チャンクサイズ: 2000, オーバーラップ: 100
   - 最大30MB, 4000チャンク

3. **a03_cc_news** (L107-117): CC NewsデータセットQ&A（カバレッジ改良方式 - ルールベース生成）
   - ファイル: `qa_output/a03_qa_pairs_cc_news.csv`
   - テキストカラム: `question`
   - チャンクサイズ: 2000, オーバーラップ: 100
   - 最大30MB, 4000チャンク

4. **a03_livedoor** (L118-128): LivedoorデータセットQ&A（カバレッジ改良方式 - ルールベース生成）
   - ファイル: `qa_output/a03_qa_pairs_livedoor.csv`
   - テキストカラム: `question`
   - チャンクサイズ: 2000, オーバーラップ: 100
   - 最大30MB, 4000チャンク

5. **a10_cc_news** (L129-139): CC NewsデータセットQ&A（ハイブリッド生成方式）
   - ファイル: `qa_output/a10_qa_pairs_cc_news.csv`
   - テキストカラム: `question`
   - チャンクサイズ: 2000, オーバーラップ: 100
   - 最大30MB, 4000チャンク

6. **a10_livedoor** (L140-150): LivedoorデータセットQ&A（ハイブリッド生成方式）
   - ファイル: `qa_output/a10_qa_pairs_livedoor.csv`
   - テキストカラム: `question`
   - チャンクサイズ: 2000, オーバーラップ: 100
   - 最大30MB, 4000チャンク
...
- CSVファイルは `qa_output/` ディレクトリに配置
- 必須カラム: `question` (全データセット共通)
- ファイル命名規則:
  - `a02_qa_pairs_cc_news.csv` - CC News LLM生成Q&A
  - `a02_qa_pairs_livedoor.csv` - Livedoor LLM生成Q&A
  - `a03_qa_pairs_cc_news.csv` - CC News ルールベース生成Q&A
  - `a03_qa_pairs_livedoor.csv` - Livedoor ルールベース生成Q&A
  - `a10_qa_pairs_cc_news.csv` - CC News ハイブリッド生成Q&A
  - `a10_qa_pairs_livedoor.csv` - Livedoor ハイブリッド生成Q&A
