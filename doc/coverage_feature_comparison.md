...
graph TD
    A[CSVデータ読込] --> B[チャンク作成]
    B --> C[小チャンク統合<br/>150-400トークン]
    C --> D[バッチ処理<br/>3-5チャンク]
    D --> E[LLM呼び出し<br/>GPT-5-mini]
    E --> F[Q/Aペア生成]
    F --> G[多段階カバレージ分析]
    G --> H[結果出力]
```
...
```mermaid
graph TD
    A[CSVデータ読込] --> B[MeCabキーワード抽出]
    B --> C[テンプレートマッチング]
    C --> D[ルールベースQ/A生成]
    D --> E[バッチ埋め込み<br/>最大2048個]
    E --> F[カバレージ計算]
    F --> G[結果出力]
```
...
```mermaid
graph LR
    A[現状] --> B[Phase 1<br/>機能移植]
    B --> C[Phase 2<br/>最適化]
    C --> D[Phase 3<br/>統合版リリース]
